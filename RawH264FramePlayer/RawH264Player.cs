...
namespace RawH264FramePlayer
{
    public class RawH264Player : IDisposable
    {
        ...
        public RawH264Player(string rawH264Folder)
        {
            ...
            {
                throw new ArgumentException("指定されたH264ファイル格納フォルダが存在しません。");
            }
        ...
        public LinkedList<(DateTime startFrameTime, DateTime endFrameTime)> SearchRecordedTerms(DateTime searchStartTime, DateTime searchEndTime)
        {
            ...
            {
                if (recordedFrameEndTime < searchStartTime) break;
                if (searchEndTime < recordedFrameStartTime) break;
